...
export async function GET(request: NextRequest) {
  ...
  if (!code || !state || !stateCookie || state !== stateCookie) {
    return NextResponse.redirect(`${base}/admin?error=state`);
  }

  const login = await loginDesdeCodigo(code);
  if (!login) {
    return NextResponse.redirect(`${base}/admin?error=github`);
  }

  if (!(await accesoPermitido(login))) {
    return NextResponse.redirect(`${base}/admin?error=no_org`);
  }
